...
const { listScraperKeys } = require('./registry');
...
function validateConfig(data) {
  const SCRAPER_KEYS = listScraperKeys();
...
function resolveTarget(targets, arg) {
  ...
  return (
    targets.find((t) => String(t.id).toLowerCase() === low) ||
    targets.find((t) => (t.label && String(t.label).includes(s)) || false)
  );
}
