...
4. **注册**  
   - 在 **`lib/orchestrator/registry.js`** 的 **`SCRAPER_TO_SCRIPT_REL`** 增加：`站点标识: path.join('gaode', '<目录>', '<脚本>.js')`（相对项目根）。  
...
| scraper 键 | 与 `lib/orchestrator/registry.js` 的 `SCRAPER_TO_SCRIPT_REL` 一致；缺省为 `book18` |
...
- 忘记在 **`lib/orchestrator/registry.js`** 的 **`SCRAPER_TO_SCRIPT_REL`** 注册 → workflow 报未知 scraper。
