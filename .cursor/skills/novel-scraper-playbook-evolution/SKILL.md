...
description: >-
  ...
  retry, deciding what to document in SKILL vs 根目录/gaode 说明文档 vs code only, or
...
5. **回写知识**：见下节「写在哪里」— 能让下一次代理／人类**少试错**的才写进 skill 或 **`gaode/<站点>/说明文档.md`**（全项目约定仍写根目录 **`说明文档.md`**）。
...
| 全项目约定（输出目录、manifest、workflow 键） | **`novel-scraper-site-adapter`** + 根目录 **`说明文档.md`** |
...
**原则**：Skill 里写**决策与顺序**；程序里写**事实与数值**；根目录 **`说明文档.md`** 写**项目级约定与索引**；**`gaode/<站点>/说明文档.md`** 写**该站人类可读的说明与命令示例**。
...
- [ ] 若为新站型：**`gaode/<站点>/说明文档.md`**（及根目录索引表链接）是否补上目录／正文／分页／注意事项？
